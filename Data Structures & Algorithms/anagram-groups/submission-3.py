...
# Time: O(n·m)
#     outer loop runs n times (once per string)
#     inner loop runs up to m times (once per character)
#     tuple(count) is always O(26) → constant, ignored
#     so total: O(n·m)

# Space: O(n·m)
#     res stores all strings in their original form
#     in the worst case all strings are unique (no anagrams) → n keys, each holding 1 string of length m
#     so total space for stored strings: O(n·m)
#     the count array is always 26 ints → constant, ignored
